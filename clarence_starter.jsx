...
export default function ClarenceLemonadeClone() {
    ...
    <div className="min-h-screen bg-white text-gray-900 font-sans">
      ...
      <header className="w-full border-b border-gray-100 sticky top-0 z-40 bg-white/80 backdrop-blur">
        <div className="mx-auto max-w-6xl px-6 py-5 grid grid-cols-3 items-center">
          ...
          <div className="justify-self-center">
            <img src="/clarence-logo.png" alt="Clarence Logo" className="h-8 w-auto" />
          </div>
      ...
      <section id="how" className="mx-auto max-w-6xl px-6 py-24 text-center">
        ...
        <div className="flex justify-center gap-8 flex-wrap">
          {['travelers.svg','nationwide.svg','chubb.svg','progressive.svg'].map((logo)=>(
            <img key={logo} src={`/carriers/${logo}`} alt={logo} className="h-12 opacity-60" />
          ))}
        </div>
      </section>
      ...
      <section id="privacy" className="mx-auto max-w-6xl px-6 py-24 text-center">
        ...
        <div className="mx-auto h-56 w-80 rounded-3xl border border-gray-200 bg-gray-50 flex items-center justify-center">
          <img src="/icons/lock.svg" alt="lock icon" className="w-16 opacity-60" />
        </div>
      </section>

      {/* CTA */}
      <section className="mx-auto max-w-6xl px-6 py-24 text-center bg-gray-50">
        ...
        <div className="mx-auto max-w-xl flex flex-col sm:flex-row gap-4 justify-center">
          <input className="flex-1 rounded-full border border-gray-300 px-5 py-4 text-base focus:outline-none focus:ring-2 focus:ring-[#0057d9]" placeholder="Ask me anything about your insurance" />
          <button className="rounded-full bg-[#0057d9] px-8 py-4 text-white font-semibold text-base hover:opacity-90">Get Started</button>
        </div>
...
function ChatBubble({ who = 'bot', text = '' }: { who?: 'bot' | 'user'; text: string }) {
  ...
  return (
    <div className={`mb-4 max-w-[85%] rounded-2xl px-5 py-3 text-base ${isUser ? 'ml-auto bg-[#0057d9] text-white' : 'bg-gray-50 border border-gray-200 text-gray-800'}`}>{text}</div>
  );
}
      ...
      <div className="group rounded-3xl border border-gray-300 bg-white/80 shadow-sm backdrop-blur px-4 py-3 sm:px-5 sm:py-4 flex items-center gap-3">
        ...
        <div className="hidden sm:flex items-center gap-3 pr-1 text-gray-600">
          ...
          <button
            ...
            className={`h-10 w-10 rounded-full grid place-items-center ${focused || text ? 'bg-[#0057d9] text-white' : 'bg-[#00bcd4] text-white'} shadow`}
